...
export function MemberPostsGrid({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/members/${userId}/posts`)
      .then((r) => r.json())
  ...
  if (posts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 px-4 bg-[var(--elite-bg)]">
        ...
        <p className="elite-heading font-semibold text-[var(--elite-text)] text-lg">No posts</p>
        <p className="elite-body text-sm text-[var(--elite-text-secondary)] mt-1">This member hasn’t shared any posts yet.</p>
      </div>
    );
  }
