...
export function MemberEventsClient({ userId }: { userId: string }) {
  const [events, setEvents] = useState<EventCardEvent[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`/api/members/${userId}/events`)
      .then((r) => r.json())
  ...
  if (events.length === 0) {
    return (
      <div className="flex-1 bg-[var(--elite-bg)] min-h-[40vh] flex flex-col items-center justify-center px-4">
        ...
        <p className="elite-heading font-semibold text-[var(--elite-text)] text-lg">No events</p>
        <p className="elite-body text-sm text-[var(--elite-text-secondary)] text-center mt-1">
          This member isn’t attending any upcoming events.
        </p>
        <Link
          href="/app/events"
