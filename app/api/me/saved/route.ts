...
import { connectDB, PostModel, PostSaveModel, PostLikeModel, PostCommentModel } from "@/lib/db";
...
export async function GET(request: NextRequest) {
  ...
  const saveQuery: { userId: mongoose.Types.ObjectId; _id?: { $lt: mongoose.Types.ObjectId } } = {
    userId: new mongoose.Types.ObjectId(session.userId),
  };
  if (cursor && mongoose.Types.ObjectId.isValid(cursor)) {
    saveQuery._id = { $lt: new mongoose.Types.ObjectId(cursor) };
  }

  const saves = await PostSaveModel.find(saveQuery)
    .sort({ createdAt: -1 })
    .limit(PAGE_SIZE + 1)
    .lean()
    .exec();

  const hasMore = saves.length > PAGE_SIZE;
  const list = hasMore ? saves.slice(0, PAGE_SIZE) : saves;
  const nextCursor =
  ...
  const items = list.map((s) => {
    const save = s as unknown as { postId: mongoose.Types.ObjectId };
    const id = String(save.postId);
