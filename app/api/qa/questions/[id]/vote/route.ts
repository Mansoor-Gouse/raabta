...
import { connectDB, QaQuestionVoteModel, QuestionModel } from "@/lib/db";
...
export async function POST(
  ...
  async function applyVote() {
    const existing = await QaQuestionVoteModel.findOne({ questionId, userId }).exec();
    const prev = existing ? (existing.value as 1 | -1) : 0;
    const next = value === 0 ? 0 : (value as 1 | -1);
    const delta = next - prev;

    if (existing && next === 0) {
      await QaQuestionVoteModel.deleteOne({ _id: existing._id }).exec();
    } else if (existing && next !== 0) {
      await QaQuestionVoteModel.updateOne({ _id: existing._id }, { $set: { value: next } }).exec();
    } else if (!existing && next !== 0) {
      await QaQuestionVoteModel.create({ questionId, userId, value: next });
    }

    if (delta !== 0) {
      await QuestionModel.updateOne({ _id: questionId }, { $inc: { score: delta } }).exec();
    }

    const updated = await QuestionModel.findById(questionId).select("score").lean().exec();
