...
export function MediaSelector({
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-auto p-4">
        {items.length === 0 ? (
          <div
            ...
            onDrop={handleDrop}
            className="w-full min-h-[280px] rounded-xl border-2 border-dashed border-[var(--ig-border)] flex flex-col items-center justify-center gap-3 text-[var(--ig-text-secondary)] hover:border-[var(--ig-link)] hover:bg-[var(--ig-bg)] transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-[var(--ig-link)] focus:ring-offset-2"
          >
            ...
            <span className="text-xs text-[var(--ig-text-tertiary)]">or drag and drop</span>
            <input
              ref={inputRef}
              type="file"
              accept={ACCEPT}
          ...
          >
            <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
              {items.map((item, i) => (
                <div
                  key={i}
                  className="relative shrink-0 w-20 h-20 rounded-lg overflow-hidden bg-[var(--ig-border-light)] group"
                >
                  {item.type === "image" ? (
      ...
      {items.length > 0 && (
        <div className="p-4 border-t border-[var(--ig-border-light)]">
          <button
            type="button"
            onClick={onNext}
            className="w-full py-2.5 rounded-lg bg-[var(--ig-link)] text-white text-sm font-semibold hover:opacity-90"
          >
            Next
          </button>
        </div>
