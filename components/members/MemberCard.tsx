    ...
    <article
      className={`elite-events block rounded-[var(--elite-radius-lg)] border border-[var(--elite-border)] bg-[var(--elite-card)] transition-all duration-[var(--elite-transition)] elite-hover-lift focus-within:ring-2 focus-within:ring-[var(--elite-accent)] ${
        compact
          ...
          : "p-2.5 hover:border-[var(--elite-accent-muted)] hover:shadow-[var(--elite-shadow)]"
      }`}
    >
      <div className={`flex items-center gap-2 ${compact ? "flex-col items-stretch" : "gap-2 min-h-0"}`}>
        ...
        <div className={`min-w-0 flex-1 ${compact ? "" : "flex items-center gap-2"}`}>
          <div className="min-w-0 flex-1">
            <Link href={`/app/members/${member.id}`} className="block truncate">
            ...
            {compact && (member.circleTypeForMe === "INNER" || member.circleTypeForMe === "TRUSTED") && (
