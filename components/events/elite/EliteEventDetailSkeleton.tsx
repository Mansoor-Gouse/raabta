...
/** Static header + skeleton body for event detail loading route (instant header on nav) */
export function EventDetailLoadingShell() {
  return (
    <div className="elite-events min-h-full bg-[var(--elite-bg)] flex flex-col">
      ...
      <EliteEventDetailSkeletonBody />
    </div>
  );
}

/** Body only (no header); use with static header for instant nav + skeleton transition */
export function EliteEventDetailSkeletonBody() {
  return (
    <div className="flex-1 overflow-y-auto transition-opacity duration-200" aria-busy="true" aria-label="Loading event">
